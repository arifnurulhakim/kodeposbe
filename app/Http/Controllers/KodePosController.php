...
class KodePosController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            // Hitung total data yang sesuai dengan parameter pencarian
           
    
            // Eksekusi kueri pada objek $kodepos dengan filter pencarian
            $data = $kodepos->when($search, function ($query) use ($search) {
                return $query->where(function ($innerQuery) use ($search) {
                    $innerQuery->where('kode_pos.kode_dagri', 'like', '%' . $search . '%')
                        ->orWhere('kode_pos.kode_old', 'like', '%' . $search . '%')
                        ->orWhere('kode_pos.kode_mod', 'like', '%' . $search . '%')
                        ->orWhere('kode_pos.kode_new', 'like', '%' . $search . '%')
                        ->orWhere('desas.nama_desa', 'like', '%' . $search . '%')
